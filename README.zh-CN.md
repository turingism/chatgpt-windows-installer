...
启动器还会将当前安装进程切换为 UTF-8，并统一 PowerShell 控制台与原生命令的输入输出编码，避免本地化的 `winget` 和 Windows 中文信息显示为乱码。
安装过程中，Windows 会显示整体动态进度条，终端也会保留带时间戳的 `[PROGRESS xx%]` 进度行。脚本会明确提示静默安装器已经启动、进程返回码、验证状态以及 ChatGPT 启动请求是否已由 Windows 接收，即使没有弹出独立安装窗口也能判断当前状态。
...
| `.github/workflows/validate.yml` | Windows PowerShell 5.1 解析和演练 CI |
