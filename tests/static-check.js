...
requireText(ps, "@('py.exe', 'py', 'python.exe', 'python')", 'Python Launcher 与解释器验证命令');
requireText(ps, "'Programs\\Python\\Python314\\python.exe'", 'Python 用户级安装路径验证');
requireText(ps, "'Python 3.14*'", 'Python 注册表验证');
requireText(ps, 'foreach ($verificationAttempt in 1..3)', '安装后验证重试');
requireText(ps, "'Microsoft.DotNet.SDK.10'", '.NET SDK 包');
requireText(ps, "'GitHub.cli'", 'GitHub CLI 包');
requireText(ps, 'New-Object System.Diagnostics.ProcessStartInfo', '隔离 GitHub 登录状态检查');
requireText(ps, '$startInfo.RedirectStandardError = $true', 'GitHub 登录检查错误流捕获');
requireText(ps, '& $GitHubCliPath auth login --hostname github.com --git-protocol https --web', 'GitHub 浏览器登录');
requireText(ps, "Set-InstallerProgress -Percent 93 -Stage 'GitHub authentication'", 'GitHub 登录进度');
requireText(ps, '[switch]$SkipGitHubLogin', '跳过 GitHub 浏览器登录参数');
requireText(ps, "Add-Result -Component 'GitHub authentication' -Status 'User action required'", 'GitHub 未登录用户操作状态');
requireText(ps, "-Component 'ChatGPT account sign-in'", 'ChatGPT 账号登录节点');
requireText(ps, "-Component 'ChatGPT connectors and plugins'", 'ChatGPT 连接器授权节点');
...
requireText(ps, "Write-Progress `", 'Windows 动态进度条');
requireText(ps, "'[{0}] [PROGRESS {1,3}%]", '持久化百分比进度行');
requireText(ps, "Set-InstallerProgress -Percent 0", '安装开始进度');
requireText(ps, "Set-InstallerProgress -Percent 99", '报告写入进度');
requireText(ps, 'Complete-InstallerProgress', '安装完成进度');
requireText(ps, 'Silent package installers may not open a separate window.', '静默安装启动说明');
requireText(ps, 'Installer process returned exit code', '安装进程返回说明');
requireText(ps, 'Launch request accepted by Windows.', 'ChatGPT 启动确认');
...
requireText(workflow, '-Profile Complete -DryRun -NoLaunch', 'Windows CI 完整模式演练');
...
forbidText(ps, /&\s*\$gh\.Source\s+auth\s+status/i, '会被严格错误模式误判的 GitHub 登录检查');
forbidText(ps, /return\s+'installed'/i, '命令异常时的安装成功误判');
